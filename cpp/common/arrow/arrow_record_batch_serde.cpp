...
ArrowRecordBatchSerde::deserialize_from(const std::string &name, const metaspore::serving::PredictRequest &request) {
    ...
    if (!rb_reader_result.ok()) {
        return absl::InternalError(
            fmt::format("Create RecordBatchFileReader failed {}", rb_reader_result.status().ToString()));
    }
    ...
    if (!result.ok()) {
        return absl::InternalError(fmt::format("Reader record batch failed {}", result.status().ToString()));
    }
    return *result;
}
