...
// ---------------------------------------------------------------------------
// Case 13: list<string> with empty string elements ([""])
// ---------------------------------------------------------------------------
TEST(LightweightListStringTest, ListStringWithEmptyElements) {
    // Row 0: [""]        — list with one empty string
    // Row 1: ["a", ""]  — mixed non-empty and empty
    // Row 2: ["", "b"]  — empty first
    auto str_builder = std::make_shared<arrow::StringBuilder>();
    arrow::ListBuilder list_builder(arrow::default_memory_pool(), str_builder,
                                    std::make_shared<arrow::ListType>(arrow::utf8()));
    ...
    auto schema = arrow::schema(
        {arrow::field("top5_list", std::make_shared<arrow::ListType>(arrow::utf8()))});
    auto batch = arrow::RecordBatch::Make(schema, 3, {arr});

    const std::string schema_src = "top5_list\n";
    auto arrow_rb = run_arrow_exec(schema_src, batch);
    auto lw_rb    = run_lightweight(schema_src, batch);
    assert_batches_equal(arrow_rb, lw_rb);
}
...
// ---------------------------------------------------------------------------
// Case 14: list<string>#string where list contains empty string elements
// ---------------------------------------------------------------------------
TEST(LightweightListStringTest, ListStringWithEmptyElementsCrossString) {
    auto str_builder = std::make_shared<arrow::StringBuilder>();
    arrow::ListBuilder list_builder(arrow::default_memory_pool(), str_builder,
                                    std::make_shared<arrow::ListType>(arrow::utf8()));
    ASSERT_OK(list_builder.Append());
    ASSERT_OK(str_builder->Append("app1"));
    ASSERT_OK(str_builder->Append(""));
    ASSERT_OK(str_builder->Append("app3"));

    ASSERT_OK(list_builder.Append());
    ASSERT_OK(str_builder->Append(""));

    ASSERT_OK_AND_ASSIGN(auto list_arr, list_builder.Finish());

    arrow::StringBuilder pkg_builder;
    ASSERT_OK(pkg_builder.Append("com.a"));
    ASSERT_OK(pkg_builder.Append("com.b"));
    ASSERT_OK_AND_ASSIGN(auto pkg_arr, pkg_builder.Finish());

    auto schema = arrow::schema({
        arrow::field("top5_list", std::make_shared<arrow::ListType>(arrow::utf8())),
        arrow::field("pkg", arrow::utf8())});
    auto batch = arrow::RecordBatch::Make(schema, 2, {list_arr, pkg_arr});

    const std::string schema_src = "top5_list#pkg\n";
    auto arrow_rb = run_arrow_exec(schema_src, batch);
    auto lw_rb    = run_lightweight(schema_src, batch);
    assert_batches_equal(arrow_rb, lw_rb);
}
